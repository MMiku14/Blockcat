...
impl<T, const N: usize> Producer<'_, T, N> {
    ...
    /// 批量推入。要么全部写入成功（单次 Release fence），要么全部拒绝。
    /// 比逐元素 push 更高效：只在末尾发射一次 Release store。
    #[inline]
    pub fn push_slice(&mut self, src: &[T]) -> Result<(), SpscError>
    where
        T: Copy,
    {
        let ring = self.ring;
        let tail = ring.tail.value.load(Ordering::Relaxed);
        let head = ring.head.value.load(Ordering::Acquire);
        let available = N - tail.wrapping_sub(head);
        if available < src.len() {
            return Err(SpscError::Full);
        }
        for (i, &v) in src.iter().enumerate() {
            // SAFETY: available >= src.len()，所有目标槽位对生产者独占
            unsafe { (*ring.buf[tail.wrapping_add(i) % N].get()).write(v) };
        }
        ring.tail
            .value
            .store(tail.wrapping_add(src.len()), Ordering::Release);
        Ok(())
    }
...
impl<T, const N: usize> Consumer<'_, T, N> {
        ...
        T: Copy,
    {
        ...
        // SAFETY: [head, tail) 内的槽位已初始化；Copy 保证引用不影响所有权
        Some(unsafe { (*ring.buf[head % N].get()).assume_init_ref() })
    }
        ...
        T: Copy,
    {
        ...
        ring.head
            .value
            .store(head.wrapping_add(M), Ordering::Release);
        Ok(())
    }

    /// 动态长度批量拷贝弹出。比 `pop_slice_copy` 更灵活。
    /// 返回实际弹出的元素数量。
    #[inline]
    pub fn pop_slice_into(&mut self, dst: &mut [T]) -> usize
    where
        T: Copy,
    {
        let ring = self.ring;
        ...
        for i in 0..count {
            // SAFETY: [head, head+count) ⊆ [head, tail)
            dst[i] = unsafe { (*ring.buf[head.wrapping_add(i) % N].get()).assume_init() };
        }
        if count > 0 {
            ring.head
                .value
...
mod tests {
    ...
    fn spsc_pop_slice_copy() {
        ...
        assert_eq!(
            c.pop_slice_copy(&mut dst2),
            Err(SpscError::InsufficientData)
        );
    }
    ...
    fn push_slice_bulk() {
        ...
        p.push_slice(&data).unwrap();
        assert_eq!(ring.len(), 12);

        // 容量不足时整批拒绝
        let more = [99u32; 8];
        assert_eq!(p.push_slice(&more), Err(SpscError::Full));
        assert_eq!(ring.len(), 12, "失败的批推不得改变队列状态");
    ...
    fn pop_slice_into_dynamic() {
        ...
        // 只有 10 个可读，dst 能容纳 16 → 实际弹出 10
        let n = c.pop_slice_into(&mut buf);
        assert_eq!(n, 10);
        for i in 0..10 {
    ...
    fn peek_does_not_consume() {
        ...
        let (mut p, c) = unsafe { ring.split() };
        assert_eq!(c.peek(), None);
        p.push(42).unwrap();
        assert_eq!(c.peek(), Some(&42));
        assert_eq!(c.peek(), Some(&42), "peek 不得推进 head");
        assert_eq!(ring.len(), 1);
    }

    #[test]
    fn producer_available() {
    ...
    fn multithreaded_stress() {
        ...
        let producer = std::thread::spawn(move || {
            let mut p = unsafe { ring_p.producer() };
            for i in 0..COUNT {
                loop {
                    if p.push(i).is_ok() {
                        break;
        ...
        let consumer = std::thread::spawn(move || {
            let mut c = unsafe { ring_c.consumer() };
            let mut next = 0u64;
            while next < COUNT {
                if let Some(v) = c.pop() {
                    assert_eq!(v, next, "乱序：期望 {next} 得到 {v}");
                    next += 1;
                } else {
                    std::hint::spin_loop();
                }
